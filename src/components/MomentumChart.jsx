...
const MomentumChart = ({ externalHistory, stats, graphRange, setGraphRange }) => {
  const chartData = useMemo(() => {
    ...
    if (graphRange === 7) {
        ...
        startDate.setDate(todayMidnight.getDate() - 364);
    }
    ...
    const baseAvg = stats.avgNumeric; // The "Lifetime Average"
    ...
    while (iterator <= todayMidnight) {
        ...
        if (!isPreHistory) {
            ...
            dailyDelta = (played - baseAvg) * 3600; // Convert to seconds
        }
        ...
        points.push({
            ...
            date: iterator.toLocaleDateString('en-US', { weekday: 'narrow' }),
    ...
    // We ensure the range is at least 60 seconds so tiny 1s changes don't look huge
    let range = maxY - minY;
    ...
    return { points, pathD, areaD, zeroY, color, getX, getY };
    ...
    <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm mb-6 border border-slate-100 dark:border-slate-700">
        ...
        <div className="h-40 w-full relative">
            <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none" className="overflow-visible">
                ...
                {/* Baseline Label */}
                <text x="2" y={chartData.zeroY - 2} fontSize="4" fill="currentColor" className="text-slate-400 opacity-50 font-mono font-bold">LIFETIME AVG</text>
