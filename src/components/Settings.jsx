...
const Settings = ({ syncStatus, isFirebaseEnabled, syncId, setSyncId, handleLogout, isDark, setIsDark }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 w-full pb-24">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 mb-6">
        ...
        {!isFirebaseEnabled ? (
          <div className="text-center">
            ...
            <p className="text-xs text-slate-500 mb-2">Cloud sync is not configured.</p>
          </div>
        ) : (
          <div>
            {syncId ? (
              ...
              <>
                <label className="text-xs font-bold text-slate-500 mb-2 block">Secret Passphrase</label>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-100 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-500"
                  />
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 mb-6 flex justify-between items-center">
      ...
      <div className="px-2 text-center text-xs text-slate-400">
         Piano v52 (Modular)<br />Restored Sync
      </div>
    </div>
  );
};
