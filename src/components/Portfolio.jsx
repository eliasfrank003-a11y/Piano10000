...
export default function Portfolio({ isDark, externalHistory = [], setExternalHistory = () => {} }) {
  ...
  const chartData = useMemo(() => {
    ...
    while (curr <= today) {
      dayMap.set(getLocalDateKey(curr), 0);
      curr.setDate(curr.getDate() + 1);
    }

    // Populate play time from CSV/Google
    sorted.forEach(s => {
      const dStr = getLocalDateKey(s.start);
      if (dayMap.has(dStr)) {
  ...
  const filteredData = useMemo(() => {
    ...
    if (activeTab === '7D') {
      ...
      const endKey = getLocalDateKey(today);
      return chartData.filter(d => d.date >= startKey && d.date <= endKey);
    }
