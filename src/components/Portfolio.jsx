...
export default function Portfolio({ isDark }) {
  ...
  const chartData = useMemo(() => {
    ...
    // Calculate Legacy Offset
    // Days between Feb 1 2024 and Dec 17 2025
    const diffTime = firstDataDate - LEGACY_START_DATE;
    const legacyDaysCount = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
    
    let cumulativeSeconds = LEGACY_TOTAL_SECONDS;
    let totalDaysElapsed = legacyDaysCount;
    ...
    for (const [dateStr, dailySeconds] of dayMap) {
      ...
      const averageSoFar = cumulativeSeconds / totalDaysElapsed;
